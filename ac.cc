...
class TrieBuilder {
public:
    TrieBuilder(std::vector<Node>& t) : trie_(t){
        trie_.clear();
        //trie_ = std::make_shared<std::vector<Node>>();
        trie_.push_back(Node());
    }

    void Build(TextDict& text_dict);
...
template <class Node>
void TrieBuilder<Node>::Build(TextDict& text_dict) {
...
bool build(const std::vector<std::string>& keys, const std::vector<value_t>& values, AC_Automata* aca)
{
    TextDict text_dict;
